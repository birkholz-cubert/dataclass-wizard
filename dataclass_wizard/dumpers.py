...
def dump_func_for_dataclass(
    ...
    skip_if_condition = get_skip_if_condition(meta.skip_if, _locals, "_skip_value")
    ...
    with fn_gen.function(
        ...
        if (_pre_dict := getattr(cls, "_pre_dict", None)) is not None:
            # class defines a `_pre_dict()`
            _locals["__pre_dict__"] = _pre_dict
            fn_gen.add_line("__pre_dict__(o)")
        elif (_pre_dict := getattr(cls_dumper, "__pre_as_dict__", None)) is not None:
...
def _asdict_inner(
    ...
    dump_func_for_cls=dump_func_for_dataclass,
) -> Any:
    cls = type(obj)
